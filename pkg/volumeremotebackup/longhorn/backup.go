...
// BuildOwnerReference creates an owner reference to a `VolumeRemoteBackup` CR.
// This ties the generated snapshot resources to the `VolumeRemoteBackup`, ensuring that the CSI snapshot
// resources (for Longhorn) are automatically deleted when the `VolumeRemoteBackup` is deleted.
// This means deleting the `VolumeRemoteBackup` will clean up the Longhorn remote backup.
...
func (lbo *LHBackupOperation) readiness(vrb *harvesterv1.VolumeRemoteBackup) (bool, error) {
	...
	if err := checkVolumeSnapshotError(vs); err != nil {
		return false, err
	}
